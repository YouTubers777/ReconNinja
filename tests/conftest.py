"""
tests/conftest.py — ReconNinja v3.2.1
Shared fixtures for all test modules.

v3.2.1 additions:
  - basic_config: now includes run_cve_lookup, ai_provider, ai_key, ai_model, nvd_key
  - full_config:  new fixture with all v3.2.1 AI/CVE fields populated
"""
import pytest
import sys
import tempfile
...
@pytest.fixture
def basic_config():
    """Standard config with v3.2.1 defaults (all new fields present)."""
    return ScanConfig(
        target="example.com",
        profile=ScanProfile.STANDARD,
        nmap_opts=NmapOptions(),
        # v3.2.1 fields — defaults
...
@pytest.fixture
def full_config():
    """Full-suite config with all v3.2.1 AI/CVE fields populated."""
    return ScanConfig(
        target="example.com",
        ...
        run_cve_lookup=True,    # v3.2.1
        ai_provider="groq",     # v3.2.1
        ai_key="gsk_testkey",   # v3.2.1
        ai_model="llama3-70b",  # v3.2.1
        nvd_key="nvd_testkey",  # v3.2.1
        threads=20,
        masscan_rate=5000,
    )
